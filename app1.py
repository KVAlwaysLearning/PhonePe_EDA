...
for q_idx, active_tab in enumerate(quarter_tabs):
    with active_tab:
        ...
        if domain_selector == "Case 1: Core Payments":
            ...
            if sub_df.empty:
                ...
                with c1:
                    ...
                    # Changed columns to match your exact extraction keys ('Count')
                    v_data = sub_df.groupby('State')['Count'].sum().sort_values(ascending=False).reset_index()
                    fig = px.bar(v_data, x='State', y='Count', color='Count', color_continuous_scale='purples')
                    st.plotly_chart(fig, use_container_width=True)
                with c2:
                    st.markdown("##### 1.2 Bivariate: Mix by Payment Type")
                    # Changed columns to match your exact extraction keys ('Count')
                    m_data = sub_df.groupby('Type')['Count'].sum().reset_index()
                    fig = px.pie(m_data, values='Count', names='Type', hole=0.3)
                    st.plotly_chart(fig, use_container_width=True)
                with c3:
                    st.markdown("##### 1.3 Multivariate: Value Trends Over Time")
                    # Changed columns to match your exact extraction keys ('Amount')
                    t_data = sub_df.groupby(['Year', 'Type'])['Amount'].sum().reset_index()
                    fig = px.line(t_data, x='Year', y='Amount', color='Type', markers=True)
                    st.plotly_chart(fig, use_container_width=True)
        ...
        # ==========================================
            ...
            if sub_df.empty:
                ...
                with c1:
                    ...
                    # Changed column to 'Count' [cite: 85]
                    b_dist = sub_df.groupby('Brand')['Count'].sum().sort_values(ascending=False).reset_index()
                    fig = px.bar(b_dist, x='Brand', y='Count', color='Brand')
                    st.plotly_chart(fig, use_container_width=True)
                with c2:
                    st.markdown("##### 2.2 Bivariate: Brand Share Stability")
                    # Changed column to 'Percentage' [cite: 85]
                    s_data = sub_df.groupby(['Year', 'Brand'])['Percentage'].mean().reset_index()
                    fig = px.line(s_data, x='Year', y='Percentage', color='Brand', markers=True)
                    st.plotly_chart(fig, use_container_width=True)
                with c3:
                    st.markdown("##### 2.3 Multivariate: Segmented Yearly Growth")
                    g_data = sub_df.groupby(['Year', 'Brand'])['Count'].sum().reset_index()
                    fig = px.bar(g_data, x='Year', y='Count', color='Brand', barmode='stack')
                    st.plotly_chart(fig, use_container_width=True)
        ...
        # ==========================================
            ...
            if sub_df.empty:
                ...
                with c1:
                    ...
                    # Mapped to 'Count' inside aggregated_insurance
                    fig = px.histogram(sub_df, x='Count', marginal='rug', color_discrete_sequence=['#ff7f0e'])
                    st.plotly_chart(fig, use_container_width=True)
                with c2:
                    ...
                    fig = px.bar(st_pols, x='State', y='Count', color='Count', color_continuous_scale='plasma')
                    st.plotly_chart(fig, use_container_width=True)
                with c3:
                    st.markdown("##### 3.3 Multivariate: Top 10 Premium Trends")
                    # Mapped to 'Amount' inside aggregated_insurance
                    ...
                    fig = px.bar(f_df, x='Year', y='Amount', color='State', barmode='group')
                    st.plotly_chart(fig, use_container_width=True)
        ...
        # ==========================================
            ...
            if sub_df.empty:
                ...
                with c1:
                    ...
                    # Changed 'Total_Revenue' to 'Amount' as extracted from map_transaction [cite: 87]
                    st_rev = sub_df.groupby('State')['Amount'].sum().sort_values(ascending=False).reset_index()
                    fig = px.bar(st_rev, x='State', y='Amount', color='Amount', color_continuous_scale='viridis')
                    st.plotly_chart(fig, use_container_width=True)
                with c2:
                    st.markdown("##### 4.2 Bivariate: National Scale Timeline")
                    yr_rev = sub_df.groupby('Year')['Amount'].sum().reset_index()
                    fig = px.line(yr_rev, x='Year', y='Amount', markers=True)
                    st.plotly_chart(fig, use_container_width=True)
                with c3:
                    ...
                    fig = px.line(f_df, x='Year', y='Amount', color='State', markers=True)
                    st.plotly_chart(fig, use_container_width=True)
        ...
        # ==========================================
            ...
            # Using map_user for district engagement
            sub_df = slice_dataframe(dfs['map_user'], target_q=current_quarter)
            
            if sub_df.empty:
                st.warning("Empty matrix slice caught.")
            else:
                # Based on map_user extraction layout, variables are 'Count' (App Opens/Users) [cite: 85]
                c1, c2, c3 = st.columns(3)
                with c1:
        ...
        # ==========================================
            ...
            if sub_df.empty:
                ...
                with c3:
                    ...
                    fig = px.bar(f_df, x='Quarter', y='Count', color='State', barmode='group')
                    st.plotly_chart(fig, use_container_width=True)
        ...
        # ==========================================
            ...
            if sub_df.empty:
                ...
                with c1:
                    ...
                    # Top transaction keys are 'Amount' and 'Count' [cite: 89]
                    st_perf = sub_df.groupby('State')['Amount'].sum().sort_values(ascending=False).reset_index()
                    fig = px.bar(st_perf, x='State', y='Amount', color='Amount', color_continuous_scale='viridis')
                    st.plotly_chart(fig, use_container_width=True)
                with c2:
                    st.markdown("##### 7.2 Bivariate: Volume vs Value Velocity")
                    fig = px.scatter(sub_df, x='Count', y='Amount', opacity=0.4, color_discrete_sequence=['green'])
                    st.plotly_chart(fig, use_container_width=True)
                with c3:
                    ...
                    fig = px.bar(f_df, x='State', y='Amount', color='Year', barmode='group')
                    st.plotly_chart(fig, use_container_width=True)
        ...
        # ==========================================
            ...
            if sub_df.empty:
                ...
                with c1:
                    ...
                    fig = px.histogram(sub_df, x='Count', nbins=20, color_discrete_sequence=['orange'])
                    st.plotly_chart(fig, use_container_width=True)
                with c2:
                    st.markdown("##### 8.2 Bivariate: Growth Velocities")
                    reg_y = sub_df.groupby('Year')['Count'].sum().reset_index()
                    fig = px.bar(reg_y, x='Year', y='Count', color_discrete_sequence=['#2ca02c'])
                    st.plotly_chart(fig, use_container_width=True)
                with c3:
                    ...
                    fig = px.line(reg_yq, x='Year', y='Count', color='Quarter', markers=True)
                    st.plotly_chart(fig, use_container_width=True)
        ...
        # ==========================================
            ...
            if sub_df.empty:
                ...
                with c1:
                    ...
                    fig = px.histogram(sub_df, x='Count', marginal='violin', nbins=20)
                    st.plotly_chart(fig, use_container_width=True)
                with c2:
                    ...
                    fig = px.bar(top15_st, x='State', y='Count', color='Count', color_continuous_scale='electric')
                    st.plotly_chart(fig, use_container_width=True)
                with c3:
                    ...
                    fig = px.box(f_df, x='State', y='Count', color='Quarter')
                    st.plotly_chart(fig, use_container_width=True)

# --- GLOBAL FEEDBACK LAYER ---
st.markdown("---")
st.caption("✔️ Production Ready Dashboard Engine v3.2.1 • Patched Column KeyError.")
